...
def frontpage(request):
    ...
    stories = Story.objects.filter(created_at__gte=date_from).order_by(
        "-number_of_votes"
    )[0:30]

    return render(request, "post/frontpage.html", {"stories": stories})
...
def search(request):
    ...
    return render(request, "post/search.html", {"stories": stories, "query": query})
...
def story(request, story_id):
    ...
    return render(request, "post/detail.html", {"story": story, "form": form})


def newest(request):
    stories = Story.objects.all()[0:200]

    return render(request, "post/newest.html", {"stories": stories})
...
def vote(request, story_id):
    ...
    if story.created_by != request.user and not Vote.objects.filter(
        created_by=request.user, story=story
    ):
